...
def run_mpc_cifar(
    ...
    # Data loading code
    def preprocess_data(context_manager, data_dirname, download):
        ...
        with context_manager:
            trainset = datasets.CIFAR10(
                data_dirname, train=True, download=download, transform=transform
            )
            testset = datasets.CIFAR10(
                data_dirname, train=False, download=download, transform=transform
            )
    ...
    if os.path.isdir(data_dir):
        train_loader, val_loader = preprocess_data(context_manager, data_dir, False)
    else:
        data_dir = tempfile.TemporaryDirectory()
        train_loader, val_loader = preprocess_data(context_manager, data_dir.name, True)

    if evaluate:
        if not skip_plaintext:
            logging.info("===== Evaluating plaintext LeNet network =====")
            validate(val_loader, model, criterion, print_freq, gpu_id)
        ...
        validate(val_loader, private_model, criterion, print_freq, gpu_id)
        # logging.info("===== Validating side-by-side ======")
        # validate_side_by_side(val_loader, model, private_model)
        return

    if torch.cuda.is_available():
        model = model.to("cuda:%d" % gpu_id)
    # define loss function (criterion) and optimizer
    for epoch in range(start_epoch, epochs):
        adjust_learning_rate(optimizer, epoch, lr)

        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, print_freq, gpu_id)
...
def train(train_loader, model, criterion, optimizer, epoch, print_freq=10, gpu_id=0):
    ...
    for i, (input, target) in enumerate(train_loader):
        if torch.cuda.is_available():
            input = input.to("cuda:%d" % gpu_id)
            target = target.to("cuda:%d" % gpu_id)
...
def validate(val_loader, model, criterion, print_freq=10, gpu_id=0):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    if torch.cuda.is_available():
        model = model.to("cuda:%d" % comm.get().get_rank())
